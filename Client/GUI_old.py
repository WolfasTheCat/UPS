...
class GameView(tk.Frame): #TODO game visualisation
    turn = True
    count = 1
...
class MenuView(tk.Frame):
    ...
    def __init__(self, root):
        ...
        b_multi  =   tk.Button(root, text= "Multiplayer", font=("Helvetica",20), height=3,width=6, bg=("white"), command=lambda: self.b_multi_clicked(root,b_multi))
        b_multi.pack(side="top", fill="both", expand=True)
        b_leave =   tk.Button(root, text= "Leave", font=("Helvetica",20), height=3,width=6, bg=("white"), command=lambda: self.b_exit_clicked(b_leave))
...
class App(tk.Tk):
    def __init__(self, *args, **kwargs):
            ...
            self.frames = {}
            for F in (StartView, LoginView, GameView, ResultView):
